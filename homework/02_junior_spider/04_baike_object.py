...
class SpiderRegex:
    ...
    # 开始爬取
    def run(self):
        for num in xrange(self.deep):
            self.get_page_index(num + 1)

if __name__ == '__main__':
    url = 'https://www.qiushibaike.com/text/page/%s/?s=4988529'
    baike_regex = re.compile("href=\"/article/(\d+?)\".*?class='contentHerf'.*?<span>(.*?)</span>", re.S)
    spy1 = SpiderRegex(url, baike_regex, deep_index=2, file_path='./bak/')
    spy1.run()
